...
class normal_payments(models.Model):
    ...
    def action_confirm(self):
        ...
        if self.send_rec_money == 'send':
            ...
            if not self.payment_subtype:
                bank_statement_id = self.env['account.bank.statement'].search(
                    [('journal_id', '=', self.payment_method.id), ('state', '=', 'open')])
                # if bank_statement_id.balance_end < self.amount1:
                #     raise exceptions.ValidationError('رصيد الخزنة لا يسمح')
        else:
        ...
        self.env['create.moves'].create_move_lines(move=move, move_line=move_line,
                                                   ...
                                                   send_rec_money=self.send_rec_money,
                                                   journal=self.payment_method.id,
        ...
        if self.payment_subtype:
            for check in self.pay_check_ids:
                ...
                check_line_val['cheque_owner'] = check.cheque_owner
                ...
                check_line_val['payment_id'] = self.id
