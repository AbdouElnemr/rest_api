...
class create_moves(models.Model):
    ...
    def create_move_lines(self, **kwargs):
        ...
        #     bank_statement_id = self.env['account.bank.statement'].search([('journal_id', '=', kwargs['journal']), ('state', '=', 'open')])
        #     print(bank_statement_id)
        #     if bank_statement_id:
        #         statement_amount = kwargs['amount']
        #         if 'send_rec_money' in kwargs and kwargs['send_rec_money'] and kwargs['send_rec_money'] == 'send':
        #             statement_amount = -kwargs['amount']
        #         print(statement_amount)
        #         bank_statement_id.line_ids = [(0, 0, {'date': fields.Date.today(), 'payment_ref': kwargs['move']['name'], 'partner_id': kwargs['move_line']['partner_id'], 'amount': statement_amount})]
        #         print(bank_statement_id.line_ids)
        # move.post()
