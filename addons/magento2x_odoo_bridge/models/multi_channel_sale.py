...
class MultiChannelSale(models.Model):
    ...
    def export_magento2x(self, exp_obj, **kwargs):
        ...
        if model_name == 'product.template':
            ...
            if store_template_id and store_variants_id:
                result = response_object(model_name,store_template_id,store_variants_id, store_default_code)
            else:
                result = False,False
    ...
    def update_magento2x(self, record, get_remote_id, **kwargs):
        ...
        if store_id:
            if model_name == 'product.category':
                result = self.env['export.categories'].magento2x_post_categories_data(sdk,channel_id,record,"update")
